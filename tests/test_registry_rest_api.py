...
from micro_registry.registry_rest_api import (
    RegistryRestApi,  # Ensure this import matches the actual location of your RestApi class
)
...
class TestRestApi(unittest.TestCase):
    ...
    def test_load_instances_from_yaml_string(self):
        ...
        """
        response = self.client.post(
            self.prefix + "/load-instances-from-yaml/",
    ...
    def test_load_module(self):
        ...
        response = self.client.post(
            self.prefix + "/load-module/", json={"file_path": file_path}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_load_modules_from_directory(self):
        ...
        response = self.client.post(
            self.prefix + "/load-modules-from-directory/", json={"directory": test_path}
        )
        self.assertEqual(response.status_code, 200)

    def test_get_classes_by_base_class(self):
        response = self.client.get(
            self.prefix + "/classes-by-base/",
            params={"base_class_name": "MicroComponent"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("TestComponent", response.json()["classes"])
    ...
    def test_get_class_names_by_base_class(self):
        response = self.client.get(
            self.prefix + "/class-names-by-base/",
            params={"base_class_name": "MicroComponent"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_instances_by_base_class(self):
        ...
        response = self.client.get(
            self.prefix + "/instance/attribute_instance_new/attributes/"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["attributes"]["value"]["value"], 99)

        response = self.client.get(
            self.prefix + "/filter-instances-by-base-class/",
            params={"base_class_name": "MicroComponent"},
        )
        self.assertEqual(response.status_code, 200)
