...
class TestRegistryRestApi(unittest.TestCase):
    def setUp(self):
        ...
        self.test_component = ComponentWithFloatList(
            name="TestComponentWithFloatList", float_values=[1.1, 2.2, 3.3]
        )
    ...
    def test_get_instance_attributes_with_float_list(self):
        ...
        response = self.client.get(
            "/api/v1/instance/TestComponentWithFloatList/attributes/"
        )
        self.assertEqual(response.status_code, 200)

        expected_attributes = {
            "attributes": {
                "name": {"type": "str", "value": "TestComponentWithFloatList"},
                "float_values": {
                    "type": "list",
                    "value": [1.1, 2.2, 3.3],
                    "is_property": True,
        ...
        self.assertDictEqual(
            response.json()["attributes"]["name"],
            expected_attributes["attributes"]["name"],
        )
        self.assertDictEqual(
            response.json()["attributes"]["float_values"],
            expected_attributes["attributes"]["float_values"],
        )
    ...
    def test_update_float_list_property(self):
        ...
        response = self.client.post(
            "/api/v1/instance/TestComponentWithFloatList/attributes/update/",
            json=update_payload,
        )
        self.assertEqual(response.status_code, 200)
