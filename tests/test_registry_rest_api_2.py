...
class TestRegistryRestApi(unittest.TestCase):
    ...
    def test_get_instance_attributes(self):
        ...
        expected_attributes = {
            "attributes": {
                "app": {"type": "FastAPI", "value": "<non-serializable: FastAPI>"},
                "host": {"type": "str", "value": "127.0.0.1"},
                "port": {"type": "int", "value": 8001},
                "children": {
                    "type": "list",
                    "items": [
                        {"type": "MicroComponent", "component_name": "ChildComponent"}
                    ],
                },
                "parent": {
                    "type": "MicroComponent",
        ...
        self.assertTrue(
            is_subset(expected_attributes, response.json()),
            "expected is not contained within response",
        )
    ...
    def test_update_instance_attributes(self):
        ...
        response = self.client.post(
            "/api/v1/instance/TestComponent/attributes/update/", json=update_payload
        )
        self.assertEqual(response.status_code, 200)

        # Verify that attributes have been updated
        updated_attributes = self.client.get(
            "/api/v1/instance/TestComponent/attributes/"
        ).json()["attributes"]
    ...
    def test_update_invalid_attribute(self):
        ...
        update_payload = {"attributes": {"non_existent_attr": {"value": "value"}}}
        response = self.client.post(
            "/api/v1/instance/TestComponent/attributes/update/", json=update_payload
        )
        self.assertEqual(response.status_code, 400)
        self.assertIn(
            "Attribute 'non_existent_attr' not found", response.json()["detail"][0]
        )
    ...
    def test_partial_update_instance_attributes(self):
        ...
        update_payload = {
            "attributes": {
                "port": {
                    "value": 8001
                },  # This should not cause an update since it's the same
                "name": {
                    "value": "PartiallyUpdatedComponent"
        ...
        self.assertEqual(
            updated_attributes["name"]["value"], "PartiallyUpdatedComponent"
        )  # updated
