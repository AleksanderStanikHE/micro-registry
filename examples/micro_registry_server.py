...
from micro_registry.registry import create_instance, instance_registry, register_class
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="127.0.0.1",
        help="Host for the Registry REST API",
    )
    parser.add_argument(
        "--registry-port", type=int, default=8001, help="Port for the Registry REST API"
    )
...
def main():
    ...
    print(
        f"Registry REST API is running on http://{args.registry_host}:{args.registry_port}"
    )
